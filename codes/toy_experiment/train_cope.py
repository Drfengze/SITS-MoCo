...
from cope_transformer import *  # 确保这里导入的是COPE的transformer模型
...
config.input_dim = 4  # 根据你的数据设置
config.num_classes = 1  # 对于回归任务，这通常是1
...
plt.plot(test_ground_truth[n, :, 0].numpy(), label='Ground Truth')
plt.plot(model_output[n, :, 0].cpu().numpy(), label='Prediction')
plt.legend()
